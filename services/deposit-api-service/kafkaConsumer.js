...
// const cashbackModel = require('../models/cashbackModel');
// const {addAmountTransaction} = require("../services/paymentService");
...
//                   const paymentProcess = await PaymentProcessModel.findOneAndUpdate(
...
//                       selfTransferModel.create(selfTxnData),
...
//                   let paymentData = await PaymentProcessModel.findOne({ txnid: callbackData.data.merchantTransactionId, status: "pending" });
...
//                       let getGetAmountDeductStatus = await appSettingModel.find();
//                       let getGstAmount = 0;
...
//                         await PaymentProcessModel.findOneAndUpdate(
...
//                       let findOffer = await offerModel.findOne({ _id: mongoose.Types.ObjectId(paymentData.offerid) });

//                       // Check if the offer is already used
//                       let alreadyUsed = await usedOfferModel.find({
