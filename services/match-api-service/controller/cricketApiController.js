...
class cricketApiController {
  constructor() {
    return {
      ...
      importMatchPlayers: this.importMatchPlayers.bind(this),
  ...
  async importMatchPlayers(req, res) {
    try {
      // console.log("req.params.matchkey",req.params.matchkey);
      const fantasyTypeMOdel = require("../../../models/fantasyModels.js");
      const getSportName = await fantasyTypeMOdel.findOne(
    ...
    for (const player of allPlayers) {
      ...
      if (insertPlayer) {
        ...
        let matchPlayers = {
          ...
          "team_logo": teamData?.logo || `${global.constant.IMAGE_URL}avtar1.png`,
        ...
        var expRedisTime = await matchTimeDifference(matchkey);
        await redisjoinTeams.setkeydata(keyName, matchPlayerData, expRedisTime);
      }
    }
