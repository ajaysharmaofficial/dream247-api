...
// const p2pTransactionModel = require("../models/p2pTransaction.Model.js");
...
//                 const alreadyExistingWithdraw = await WithdrawModel.findOne({ withdraw_req_id: withdrawalData.withdraw_req_id });
...
//                 const addedWithdraw = await WithdrawModel.findOneAndUpdate(
...
//                 let lastTxn = await TransactionModel.findOne({ userid: user._id }).sort({ createdAt: -1 }).select('total_available_amt');
//                 console.log("lastTxn", lastTxn);

//                 let currentTxn = await TransactionModel.findOneAndUpdate(
...
//                   if (Number(withdrawalData.amount) <= 0) {
//                     // If amount is zero or less, mark as processed without payout
...
//                     console.log(`🛑 Skipped payout for ${withdrawReqId} due to zero amount.`);
//                     return message.offset;
//                   }
...
//                       `${global.constant.RAZORPAY_X_KEY_ID_LIVE}:${global.constant.RAZORPAY_X_KEY_SECRET_LIVE}`
//                     ).toString("base64");
                  
//                     const sanitizeName = (name) =>
//                       name.replace(/\\+/g, "").replace(/\+/g, " ").replace(/-/g, "").trim();
                  
//                     const sanitizedName = sanitizeName(user.bank?.accountholder || "");
//                     console.log("👤 Sanitized Account Holder Name:", sanitizedName);
                  
//                     try {
//                       // Razorpay Contact
//                       let contactId = user.razorpay_x_contact_id;
//                       if (!contactId) {
//                         console.log("📞 Creating Razorpay Contact for user:", user._id);
...
//                         await userModel.updateOne({ _id: user._id }, { razorpay_x_contact_id: contactId });
//                         console.log("📝 Saved contact ID to user record");
//                       } else {
//                         console.log("🔁 Existing Razorpay Contact ID found:", contactId);
//                       }
...
//                         await userModel.updateOne({ _id: user._id }, { fund_account_id: fundAccountId });
//                         console.log("📝 Saved fund account ID to user record");
//                       } else {
//                         console.log("🔁 Existing Fund Account ID found:", fundAccountId);
//                       }
...
//                           amount: Number(withdrawalData.amount) * 100,
//                           currency: "INR",
//                           mode: paymentMode,
//                           purpose: "refund",
//                           reference_id: withdrawReqId,
...
//                             "Idempotency-Key": `payout:${withdrawReqId}`,
//                           },
//                         }
//                       );
//                       console.log("✅ Payout Success:", payoutRes.data.id);
...
//                   await TransactionModel.insertMany(data.transactionDocs);
//                   await WithdrawModel.create(data.withdrawDoc);
//                   await p2pTransactionModel.create(data.p2pDoc);
...
// //                           const alreadyExistingWithdraw = await WithdrawModel.findOne(
// //                             { withdraw_req_id: withdrawalData.withdraw_req_id }
// //                           )
